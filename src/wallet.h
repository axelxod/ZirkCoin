...
public:
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nZIRK, int64_t& nShadow) const
    {
        nZIRK = 0;
        nShadow = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredZIRKCached;
    ...
    bool GetCredit(int64_t& nCredZIRK, int64_t& nCredShadow, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredZIRK = nCredShadow = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredZIRKCached = 0;
            nCredShadowCached = 0;
            pwallet->GetCredit(*this, nCredZIRKCached, nCredShadowCached);
            fCreditSplitCached = true;
        };
        
        nCredZIRK = nCredZIRKCached;
