...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("ZirkCoin is not connected!"));
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_ZIRK_TO_ZIRK
                || rcp.txnTypeInd == TXT_ANON_TO_ZIRK)
            {
                // -- out sdc
                std::string sError;
