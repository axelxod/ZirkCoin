...
bool ShadowBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 ZIRK, 1 Shadow
        switch(rcp.txnTypeInd)
        {
            case TXT_ZIRK_TO_ZIRK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ZIRK, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_ZIRK_TO_ANON:
                formatted.append(tr("<b>%1</b> to xZIRK %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ZIRK, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to xZIRK %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ZIRK, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ZIRK:
                formatted.append(tr("<b>%1</b>, ring size %2 to ZIRK %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ZIRK, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ZIRK, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_AmountWithFeeExceedsShadowBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your shadow balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ZIRK, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
