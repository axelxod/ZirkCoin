...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"http://ajax.googleapis.com/ajax/services/feed/load?v=2.0\&q=http://discuss.zirkcoin.com/category/1.rss",
            dataType: 'jsonp'
        ...
        var format = function(tx) {
            ...
            return "<a id='"+tx.id.substring(0,17)+"' title='"+tx.tt+"' class='transaction-overview' href='#' onclick='$(\"#navitems [href=#transactions]\").click();$(\"#"+tx.id+"\").click();'>\
                                                <span class='"+(tx.t == 'input' ? 'received' : (tx.t == 'output' ? 'sent' : (tx.t == 'inout' ? 'self' : 'stake')))+" icon no-padding'>\
                                                  <i class='fa fa-"+(tx.t == 'input' ? 'angle-left' : (tx.t == 'output' ? 'angle-right' : (tx.t == 'inout' ? 'angle-down' : 'angle-right')))+" font-26px margin-right-10'></i>"
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. ZKJhV36JvEsefkXM8vDVtKtpjtQptD5hHs)" placeholder="Enter a ZirkCoin address (e.g. ZKJhV36JvEsefkXM8vDVtKtpjtQptD5hHs)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Shadow"                    ' + (unit.type == 0 ? "selected" : "") + '>ZIRK</option> \
                    <option value="1" title="Milli-Shadow (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mZIRK</option> \
                    <option value="2" title="Micro-Shadow (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;ZIRK</option> \
